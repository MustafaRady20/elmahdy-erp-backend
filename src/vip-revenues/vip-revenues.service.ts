...
import { VipRevenue, VipRevenueDocument } from './schema/vip-revenues.schema';
...
export class VipRevenuesService {
  ...
  async create(dto: CreateVipRevenueDto): Promise<VipRevenue> {
    return await this.vipRevenueModel.create(dto);
  }
  ...
  async update(
    id: string,
    dto: UpdateVipRevenueDto,
  ): Promise<VipRevenue> {
    const revenue = await this.vipRevenueModel.findByIdAndUpdate(
      id,
      dto,
      { new: true },
    );
